...
def main(argv):
    ...
    #in the fit
    params = Parameters()
    params.add('gal_sigma', value = 3., min = 0) # arcsec 
    params.add('gal_flux', value = 100., min = 0)  # total counts on the image, watch out if this is too large, can cause problems because FT transform on narrow functions. 
    params.add('e1', value = .5) #ellipticity: e1 
    params.add('e2', value = .5)#ellipticity: e2
    params.add('x0', value = 2, vary = True) #shift in x origin. 
    params.add('y0', value = 3., vary = True)     #shift in y

    #not in the fit
    params.add('SNR_gal', value = 100., vary = False) # should probably be 20 - 40  since it is what we are interested in.
    params.add('psf_flux', value = 1., vary = False)
    params.add('psf_sigma', value = 1., vary = False)

    #get image and variance from list returned
    temp_dct = drawImage(params, logger, filename, AddNoise = True, ProduceFile = True)
