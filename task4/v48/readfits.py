...
def main(argv):
    ...
    for i in range(dim):
        for j in range(dim):
            if i < num_params and j < num_params and i >= j:
                ...
                sigma_fits = res_stds[param_i]
                if i == j:
                    sigma_gauss = 1
                    x = np.linspace(defaults.EXTENT_PULL[0],
                                    defaults.EXTENT_PULL[1], 1000)
                    ax = fish_figure.add_subplot(
                        num_params, num_params,
                        num_params * i + j + 1)
                    ax.plot(x, norm_area * mlab.normpdf(x, mean[0],
                                                        sigma_gauss))

                    ax.text(.5, .4, 'sfh:' + str(round(sigma_i,
                                                       defaults.SIG_DIGITS)),
                            transform=ax.transAxes, ha='center',
                            fontsize=defaults.FONTSIZE_VALUE,
                            fontweight='bold')
                    ax.text(.5, .3, 'sft:' + str(round(sigma_fits,
                                                       defaults.SIG_DIGITS)) +
                            '+-'
                            + str(round((sigma_fits) /
                                        (math.sqrt(2 * number_fits)),
                                        defaults.SIG_DIGITS)),
                            fontsize=defaults.FONTSIZE_VALUE, ha='center',
                            transform=ax.transAxes, fontweight='bold')
                    ax.text(.5, .2, 'bfh:' +
                            str(round(fish.biases[param_names[i]],
                                      defaults.SIG_DIGITS)),
                            fontsize=defaults.FONTSIZE_VALUE,
                            transform=ax.transAxes, ha='center',
                            fontweight='bold')
                    ax.text(.5, .1, 'bft:' + str(round(biases[param_names[i]],
                                                       defaults.SIG_DIGITS)) +
                            '+-' +
                            str(round(sigma_fits / math.sqrt(number_fits),
                                      defaults.SIG_DIGITS)),
                            fontsize=defaults.FONTSIZE_VALUE, ha='center',
                            transform=ax.transAxes, fontweight='bold')
                    ...
                    ax = fish_figure.add_subplot(num_params, num_params,
                                                 num_params * i + j + 1)
                    ax.add_patch(ellip)
                    # add bound lines.
    ...
    extents = [defaults.EXTENT_PULL] * num_params
    points_plot = []
    truths = []
    plot_names = param_names.append('redchi')
