...
def drawGalaxies(params=None, id_params=None, image=False, 
    ...
    *Noise should only be added once to the image (otherwise two different noises combined), so it should go here and not in the original drawGalaxy function. 
    ...
    if id_params is None and g_parameters is None:
        ...
        id_params = GParameters.convertParams_Id(params)
...
class GParameters:
    ...
    def __init__(self, project): 
        self.omit_model = names.omit_model
        ...
        self.model_params = GParameters.convertId_Params(self.id_params,
                                                         omit=self.omit_model)

        #just for etas
        self.fit_params = self.convertModel_Fit()

        #in order.
        self.model_param_names = self.sortModelParamsNames()
    ...
    def sortModelParamsNames(self):
        """Returns params in the order _1, _2,..., and in each 
        subscript follow 
    ...
    @staticmethod
    def convertId_Params(id_params, omit = []): 
