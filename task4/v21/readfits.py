...
def main(argv):
    wdir, galaxy_file, rltsdir, snr, plots_dir, verbose, info_file, N = (
        ...
        with open(os.path.join(path,filename)) as csvfile:
            ...
            for row in reader:
                for param in row.keys():
                    ...
                    residuals[param].append(float(params[param]) - 
                                            float(row[param]))

    biases = {param:np.mean(residuals[param]) for param in residuals.keys()}

    _, variance_noise = fns.drawGalaxy(params, snr = snr, noise_seed = 0) 
