        ...
        with open(os.path.join(rltsdir,filename)) as csvfile:
            ...
            for row in reader:
                for param in row.keys(): 
                    ...
                    residuals.get(param,[]).append(residual)
                    pulls.get(param,[]).append(pull)

    biases = {param:np.mean(residuals[param]) for param in residuals.keys()}
    pull_means = {param:np.mean(pulls[param]) for param in residuals.keys()}
    res_stds = {param:np.std(residuals[param]) for param in residuals.keys()}
