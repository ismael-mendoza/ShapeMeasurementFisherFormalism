...
def main():
    ...
    parser.add_argument('-i','--info-file', action = 'store_true',
    help = 'Write relevant information to a .txt file for future reference')

    parser.add_argument('--wdir', default = names.wdir, metavar = 'DIRECTORY',
    type = str, 
    help = ('Specify a directory name where the output will be inputted and' 
            'files read from.'))

    parser.add_argument('--galaxy-file', default = names.galaxy_file, 
    metavar = 'FILENAME', type = str,
    help = 'Specify a file where galaxies will be registered or read from.')

    parser.add_argument('--plots-dir', default = names.plots_dir, 
    metavar = 'PLOTDIRECTORY', type = str,
    ...
    parser.add_argument('-p', '--partials', action = 'store_true',
    ...
    parser.add_argument('--snr', metavar = 'SNR', 
    default = cts.snr,
    ...
    g_parameters = galfun.GParameters(wdir=args.wdir, 
                                      galaxy_file=args.galaxy_file)

    fish = fisher.fisher(g_parameters=g_parameters, snr=args.snr)
    fisherplots = plotsfisher.FisherPlots(fish = fish, wdir = args.wdir,
    ...
    info = defaults.info(g_parameters, fish)
    if args.verbose: 
        for line in info.galaxy + info.fisher:
            print line

    if args.info_file:
        info_file_name = os.path.join(args.wdir, names.info + '.txt')
