...
v24
*read initial parameters from a file created by task3res.py, 
...
def main(argv):
    ...
    param_names = gal_params.keys() #may need to change this because the name is ambigous. 



    #create a dictionary with the derivatives of the model with respect to each parameter.
    Ds_gal = {param_names[i]:partialDifferentiate(func = drawGalaxy, parameter = param_names[i], step = steps[param_names[i]], psf_params = psf_params)(gal_params).array for i in range(num_params)}
