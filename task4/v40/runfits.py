...
def objFunc(fit_params, data, variance_noise, **kwargs):
    # print orig
    #print galfun.GParameters.convertFit_Model(fit_params)
...
def main(argv):
    ...
    original = galfun.drawGalaxies(params = fit_params.valuesdict(), 
                                  image = True, **nfit_params)


    image,variance_noise = galfun.addNoise(original, snr, noise_seed)
    ...
    # print (galfun.drawGalaxies(params = fit_params.valuesdict(), 
    #                             image = True, **nfit_params) - image).array.sum()
    ...
    with open(result_filename, 'w') as csvfile:
        row_to_write = galfun.GParameters.convertFit_Model(fit_params.valuesdict())
