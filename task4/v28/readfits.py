...
def main(argv):
    (project, snr, verbose, number_fits, info_file) = (
     argv[1], float(argv[2]), argv[3], float(argv[4]), argv[5])
    ...
    for i in range(fish.num_params): 
        for j in range(fish.num_params):
            ...
            if(i == j):
                ...
                ax.text(.5,.2,'bfh:' + str(round(fish.biases[fish.param_names[i]],defaults.SIG_DIGITS)), fontsize=defaults.FONTSIZE_VALUE, transform=ax.transAxes,ha='center', )
