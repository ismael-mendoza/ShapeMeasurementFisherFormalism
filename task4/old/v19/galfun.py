...
def drawGalaxies(params = None, id_params = None, image = False, 
                galaxies = None, display = False, **kwargs):
...
class Galaxies:
    def __init__(self, wdir = None, galaxy_file = None, id_params = None, params = None): 
        ...
        if wdir is not None and galaxy_file is not None:
            ...
            if not os.path.isdir(wdir):
                raise NotADirectoryError('Directory given does not exist.')

            filename = os.path.join(wdir, galaxy_file + '.csv')
            if not os.path.isfile(filename):
                raise FileDoesNotExistError('The given file name is not in the directory: ' + wdir)
    ...
    def addNoise(self, snr, noise_seed):
        ...
        variance_noise = self.image.addNoiseSNR(noise, snr,  
                                                preserve_flux =True) 
