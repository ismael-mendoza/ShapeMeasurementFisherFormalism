...
def main(argv):
    ...
    for i in range(fish.num_params):
        for j in range(fish.num_params):
            ...
            if i == j:
                sigma_gauss = 1
                x = np.linspace(defaults.EXTENT_PULL[
                                0], defaults.EXTENT_PULL[1], 1000)
                ax = fish_figure.add_subplot(
                    fish.num_params, fish.num_params,
                ...
                ax.text(.5, .3, 'sft:' + str(round(sigma_fits,
                                                   defaults.SIG_DIGITS)) + '+-' + str(round((sigma_fits) / (math.sqrt(2 * number_fits)), defaults.SIG_DIGITS)),
                        fontsize=defaults.FONTSIZE_VALUE, ha='center',
                        transform=ax.transAxes, fontweight='bold')
                ax.text(.5, .2, 'bfh:' + str(round(fish.biases[fish.param_names[i]], defaults.SIG_DIGITS)),
                        fontsize=defaults.FONTSIZE_VALUE, transform=ax.transAxes, ha='center', fontweight='bold')
    ...
    # have to transform point.values to a convenient form for the triangle
    # plot.
