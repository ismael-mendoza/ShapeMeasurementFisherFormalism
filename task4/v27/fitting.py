...
def main():
    ...
    parser.add_argument('-p', '--project', default=defaults.PROJECT_DIR, 
    metavar='PROJECT', 
    type=str,
    ...
    if args.run_fits:
        ...
        with open(os.path.join(args.wdir,defaults.SNR_FILE), 'w') as snrfile:
            snrfile.write(args.snr)
        ...
        os.system("bsub -q " + str(args.run_fits_slac) + " -J \"name[1-" + str(args.number_fits) + "]\" \"python runfits.py \$LSB_JOBINDEX " 
            + str(args.snr) + " " + str(args.wdir) + "\"")

        with open(os.path.join(args.wdir,defaults.SNR_FILE), 'w') as txtfile:
            txtfile.write(args.snr)

    elif args.produce_results:
        with open(os.path.join(args.wdir,defaults.SNR_FILE), 'r') as snrfile:
            snr = snrfile.readline()
            print snr

        os.system("python readfits.py " +  str(args.wdir) + str(snr) + str(args.verbose) + " " + str(args.number_fits) + " " + args.info_file)
