...
class FisherPlots:
    ...
    def nameImages(self):
        ...
        while(True):
            n+= 1
            filename = ''.join([names.figure_basename,str(n),
                                names.figure_extension])
    ...
    def biasMatrixValues(self):
        ...
        for i, figure in enumerate(figuresOfBiasMatrixNumbers):
            basename = self.nameImages().strip(names.figure_extension)
            filename = ''.join([basename,'_',str(i),names.figure_extension])
