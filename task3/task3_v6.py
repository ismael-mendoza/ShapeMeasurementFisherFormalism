...
def main(argv):
    ...
    steps = dict() 
    steps['gal_sigma'] = orig_params['gal_sigma'].value * .0001
    steps['gal_flux']  = orig_params['gal_flux'].value * .0001
    steps['e1'] = 1*.0001
    steps['e2'] = 1*.0001
    steps['x0'] = nx* .0001
    steps['y0'] = ny* .0001
    ...
    for i,parameter in enumerate(orig_params.keys()):
        Ds_gal[i] = partialDerive(func = drawGalaxy, params = orig_params, parameter =parameter, step = steps[parameter])
