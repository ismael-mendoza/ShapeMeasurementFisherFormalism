...
def main(argv):
    ...
    for i in range(len(orig_params.keys())):
        for j in range(len(orig_params.keys())): 
            FisherM_chi2[i][j] = .5 * partialDerive(partialDerive(chi2, orig_params, orig_params.keys()[i], orig_params.keys()[j], steps[orig_params.keys()[i]].value, steps[orig_params.keys()[j]].value, sigma_n)
    ...
    for i in range(len(Ds_gal)): 
        for j in range(len(Ds_gal)):
            if(i >= j):
                ...
                ax.set_ylabel(str(round(FisherM[i][j],5)))
