...
def partialDeriveGal(gal_image, params, parameter, step, f):
...
def PartialDeriveChi2(gal_image, chi2, params, parameter, step, sigma_n):
...
def main(argv):
    ...
    names_Ds_gal = ['Dgal_sigma', 'Dgal_flux', 'De1', 'De2', 'Dx0', 'Dy0'] 
    ...
    for i in range(len(Ds_gal)): 
        for j in range(len(Ds_gal)):
            if(i >= j):
                ...
                drawPlot(ax, Ds_gal[i] * Ds_gal[j], names_Ds_gal[i] + ',' + names_Ds_gal[j]) 
